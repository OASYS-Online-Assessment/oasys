...
ALTER TABLE
    `users`
ADD
    COLUMN IF NOT EXISTS `activity` JSON NULL
AFTER
    `options`;
...
ALTER TABLE
    `users`
ADD
    COLUMN IF NOT EXISTS `dashboard` JSON NULL;

ALTER TABLE
    `users` DROP COLUMN IF EXISTS `lastLogin`;
...
ALTER TABLE
    `itemGroups`
ADD
    COLUMN IF NOT EXISTS `info` longtext DEFAULT NULL
AFTER
    `options`;
...
ALTER TABLE
    `logins`
ADD
    COLUMN IF NOT EXISTS `parentTemplateId` int(11) DEFAULT NULL
AFTER
    `loginType`;
...
UPDATE
    `logins` AS `cloned`
    JOIN `logins` AS `template` ON `template`.`template` = 'template'
...
UPDATE
    `logins` AS `cloned`
    LEFT JOIN `logins` AS `template` ON `template`.`template` = 'template'
    AND `template`.`name` = REGEXP_REPLACE(`cloned`.`name`, '_[0-9]{8}$', '')
SET
...
SET
    `value` = LEAST(
        ...
        GREATEST(
            5,
            CASE
                WHEN CAST(`value` AS UNSIGNED) > 300 THEN ROUND(CAST(`value` AS UNSIGNED) / 1000)
                ELSE CAST(`value` AS UNSIGNED)
            END
        )
    )
WHERE
    `option` = 'sendFrequency'
    AND `encryption` = 0;
...
ALTER TABLE
    `logins`
ADD
    COLUMN IF NOT EXISTS `createdAt` timestamp(3) NULL DEFAULT NULL
AFTER
    `displayName`;
...
UPDATE
    ...
    INNER JOIN (
        ...
        FROM
            `activity`
        WHERE
            `tsFirstLoginServer` IS NOT NULL
        GROUP BY
            `loginId`
    ) AS `recordedActivity` ON `recordedActivity`.`loginId` = `dataset`.`id`
SET
    `dataset`.`createdAt` = `recordedActivity`.`firstActivity`
WHERE
...
ALTER TABLE
    `logins`
MODIFY
    COLUMN `createdAt` timestamp(3) NULL DEFAULT CURRENT_TIMESTAMP(3)
AFTER
    `displayName`;

-- Support date-filtered dataset lookup within a template.
CREATE INDEX IF NOT EXISTS `loginsParentTemplateCreatedAt` ON `logins` (`parentTemplateId`, `createdAt`);
...
SELECT
    52,
    'Upgraded database from v047 to v052'
FROM
    DUAL
WHERE
    NOT EXISTS (

...
(function($) {
	...
	const type = 'oasysTextarea';

	function registerPlugin() {
		if (typeof(plugins) !== 'undefined') {
			plugins[type] = {
				id: type,
				constructor: oasysTextarea,
	...
	/* test instance of textarea */
	function oasysTextarea(parent, options, item, currentValue, language) {
		/*
		...
		parent.each(function (idx, span) {
			...
			const html = `<textarea autocomplete="off" id='${options.code}' name='${options.code}' maxlength='${limit}'></textarea><div class='oasysTextarea_characterCounter'><span class='charCount'>0</span>&nbsp;/&nbsp;${limit}</div>`;
			span.html(html);
		...
		element.css('resize', options.resize);
		...
		function updateCharacterCounter() {
			...
			element.next().find('.charCount').html(charCount);
		}
		...
		function hasValue() {
			return validResponse(element.val());
		}

		function onBlur(e) {
			/*
				textarea value will only be pushed to queue when input field loses focus

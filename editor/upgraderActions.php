...
$db = new rixPDO($sql_db, $sql_user, $sql_password, $sql_host, '../logs/upgraderManager_errors.txt', 1, $returnData, 'error');
...
function ofChk($data, rixPDO $db, &$returnData)
{
    ...
    $hasOL = (is_dir($olDir) && count(glob($olDir . "oasys_*")) === 1);

    if ($hasOL) {
        $olfn = basename((glob(PKGDIR . "../offlinePkg/*"))[0]);
    }
...
function getFileDlInstall($data, rixPDO $db, &$returnData)
{
    ...
    if ($isOffline) {
        copy($olDir . $fileDlName, PKGDIR . $fileDlName);
    } else {
        getFileDl($data, $db, $returnData);
    }

    $foundInst = false;
    $fullFileName = PKGDIR . $fileDlName;
    $shortFileName = $fileDlName;

    $zObj = new ZipArchive;
    $res = $zObj->open($fullFileName);

    if ($res !== false || $zObj->getFromName("inst_start.php") !== false) {
        $foundInst = true;
    }
    ...
    */
    ...
    if (!$foundInst) {
        ...
        $pkgInstallZip->open($fullFileName);
        ...
        # -------------------------------------------- #
        $returnData['instLink'] = "http://" . $_SERVER['HTTP_HOST'] . $settings['JSrootURL'] . "pkg_installer/downloads/{$randInstDir}/inst_start.php";
...
function preInstallBackup($data, rixPDO $db, &$returnData)
{
    ...
    if ($bkp === "yes") {
        // export class init to utiize some function from there
        require_once '../backupRestore/export.class.php';

        $dbBkpObj = new DbOperationClass($sql_host, $sql_user, $sql_password, $sql_db);
        ...
        try {
            $fileBkpObj->ZipUp(BKPROOT, $fileBkpObj->zipFileLoc);
        } catch (Exception $e) {
            $returnData['error'] = $e->getMessage();
            exit;
        };
...
function getFileDl($data, rixPDO $db, &$returnData)
{
    ...
    if ($dlRet !== true) {
        ...
        $returnData['error'] = $dlRetJSON['error'];
        exit;
    } else {
...
function filterFileList(array $inpArr)
{
    ...
    if (!file_exists(INSTDBROOT)) {
        ...
        if (copy(INSTDBROOT . ".template", INSTDBROOT)) {
            ...
            $returnData['error'] = "Unable to initialize installer database file. Please contact the system administrator.";
        }
    }
    ...
    if (is_null($sldb)) {
        copy(INSTDBROOT . ".template", INSTDBROOT);
        exit;
    }
...
function checkInstStatus($data, rixPDO $db, &$returnData)
{
    ...
    // patch applied either successful or failed, but deactivate maintenance mode regardless
    set_mmode("all", 0, $db);
    ...
    if (preg_match("/oasys_pkg_(.+)_\d+\.zip$/", $package, $matches)) {
        ...
        $verTxt = file_get_contents(".." . DIRECTORY_SEPARATOR . "modules" . DIRECTORY_SEPARATOR . $matches[1] . DIRECTORY_SEPARATOR . "VERSION_MODULE.txt");
...
function curlGo(array $credCurlPost = [], string $retFormat = "JSON")
{
    ...
    if (isURL($settings['upgraderURL']) === false) {
    ...
    if ($retFormat === "BINZIP") {
        ...
        // init vars
        $cExecRet = true;
        $targFile = PKGDIR . $credCurlPost['fileDlName'];
        ...
        if ((curl_getinfo($cHandle, CURLINFO_CONTENT_TYPE) === 'application/json; charset=UTF-8')) {
            curl_close($cHandle);
            fclose($dlFileHandle);
        ...
        if (curl_error($cHandle) !== "") {
            $cExecRet = json_encode(['error' => 'CURL ERROR RETURNED: <strong>' . curl_error($cHandle) . '</strong>']);
            curl_close($cHandle);
            return $cExecRet;
        }
...
function clearDlTemp($data, rixPDO $db, &$returnData, $dir2kill = PKGDIR)
{
    ...
    array_map('unlink', glob("{$dir2kill}*.*"));
...
function outputJSON()
{
    global $returnData, $action, $myAuth, $db;
    ...
    if ($returnData['error'] !== false || !empty($error)) {
        if (function_exists('set_mmode')) set_mmode("all", 0, $db);

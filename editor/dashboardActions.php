...
$db = new rixPDO($sql_db, $sql_user, $sql_password, $sql_host, '../logs/dashboard.log', 1, $returnData, 'error');
...
function wiget($data, rixPDO &$db, &$returnData, userAuth &$myAuth) {}
...
function outputJSON(): void
{
    ...
    if (isset($userAD['userSettings']['skin'])) {
        $skinList = array_column(getSkins(), "id");

class videoEditor extends InteractionEditor {
	...
	browse() {
		new jsMediaPlugin('oasysVideoBrowser', {
			mediaTypes: 'video',
			hideOptions: true,
			onClose: (sender, data) => {
	...
	static generatePreview(data, lang, options = {}) {
		...
		return `<video controls src="${settings.JSrootURL}fetchMediaFile.php?fileid=${data.fileid[lang]}&amp;checksum=${data.filechecksum[lang]}" style='max-width: 100%'></video>`;
	}
